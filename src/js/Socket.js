...
// const url = 'http://192.168.0.240:8005'
...
export default class Socket {
  ...
  registerSocketEventHandlers () {
    ...
    this.socket.on('META', (res) => {
      ...
      switch (cmd) {
        ...
        case CMD.META.UPDATE:
          xdata.commit('metaStore/saveMetaUpdateDBres', res)
          break
        case CMD.META.PULL_ALL:
          xdata.commit('dexieDBStore/changeForceAfresh', false)
          break
      }
    })
    ...
    this.socket.on('FILE', (res) => {
      ...
      if (cmd === 'upload_more') {
        xdata.commit('stateStore/changeFileUploadMore', data)
      }
    })
