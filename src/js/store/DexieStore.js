...
export default {
  ...
  mutations: {
    changeForceAfresh (state, value) {
      state.forceUpdateMetadata = value
    }
  },
  actions: {
    ...
    async openDB ({state, dispatch}, message) {
      if (state.forceUpdateMetadata) {
        ...
        state.version = 1
        state.forceUpdateMetadata = false
        state.storeForceData = true
      } else {
    ...
    storeDATA ({state}, msg) {
      ...
      if (storename) {
        state.db.transaction('rw', storename, async () => {
          ...
          /*
          用来排查拉取配置不成功
          let mdtArr = state.rows.map(item => item.tableName)
          console.log(state.forceData.size,Array.from(state.forceData.keys()))
          */
          if ((state.forceData.size + 1) >= 92) {
            state.storeForceData = false
            xdata.commit('stateStore/changeShowXhint',{
              showXhint: false,
              data: null
            })
          }
